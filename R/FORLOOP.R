...
function(code, modelInfo, ignore = NULL, .env){
...
hasMatchingIndexRanges <- function (LHS, RHS, ignore = NULL){
...
replaceDeclarationIndexRanges <- function(code, new_idx_list, ignore = NULL){
  LHS <- getLHS(code)
  RHS <- getRHS(code)
  op <- code[[1]]
