...
It is not necessary to specify the dimensions in the formula when using `PRIORS` directly.
...
code <- nimbleCode({
 PRIORS(~Time + Diet, priorSpecs = pr)
})
mod <- nimbleModel(code, chick_const)
mod$getCode()
```
...
We can replicate this structure in NIMBLE model code by setting the `PRIORS` option `modMatNames = TRUE`.

```{r}
code <- nimbleCode({
 PRIORS(~Time + Diet, priorSpecs = pr, modMatNames = TRUE)
})
mod <- nimbleModel(code, chick_const)
mod$getCode()
```
