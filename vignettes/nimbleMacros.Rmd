...
The distinction is that that creates a single multivariate node, `mu[1:N]`, while the loop creates `N` separate sclar nodes. Which is best will depend on the model structure and the algorithm used with the model [as discussed in the NIMBLE user manual](https://r-nimble.org/html_manual/cha-writing-models.html#subsec:vectorized-versus-scalar-declarations). 
