...
```{r}
code <- nimbleCode({
  mu[1:N] <- LINPRED(~Time[1:N], priorSettings=NULL)
})
```
...
```{r}
code <- nimbleCode({
  mu[1:N] <- LINPRED(~Time[1:N,1], priorSettings=NULL)
})
...
```{r}
code <- nimbleCode({
  mu[1:N] <- LINPRED(~Time[1:N] + Diet[1:N], priorSettings=NULL)
})
mod <- nimbleModel(code, chick_const)
mod$getCode()
```
...
```{r}
code <- nimbleCode({
  mu[1:N] <- LINPRED(~Time[1:N], priorSettings = NULL, link = log)
})
mod <- nimbleModel(code, chick_const)
mod$getCode()
```
